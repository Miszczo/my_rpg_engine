...
| :--- | :--- | :--- |
| **Abstrakcja** | Moduł `base.py` (klasy `Item`, `Weapon`, `Character`). | Narzuca "kontrakt" używając modułu `abc`. Metody takie jak `use()` są wymuszone za pomocą dekoratora `@abstractmethod`. |
| **Hermetyzacja** | Klasa `Weapon` (atrybut `_durability` i dekorator `@property`). | Ukrywa stan wytrzymałości broni przed bezpośrednią modyfikacją, pozwalając na kontrolowane jej obniżanie jedynie przez dedykowane settery. |
| **Dziedziczenie** | Klasy `Warrior` i `Mage` (moduł `characters.py`). | Pobierają bazowe atrybuty (jak imię czy zdrowie) z klasy `Character`, umożliwiając skupienie się jedynie na zdefiniowaniu specyficznych zachowań. |
| **Polimorfizm** | Metoda `attack()` wywoływana w pętli w `Battle`. | Silnik bitwy wywołuje tę samą metodę ataku niezależnie od tego, czy celem jest obiekty klasy `Warrior` czy `Mage` – każda z klas wykonuje ją na swój własny sposób. |